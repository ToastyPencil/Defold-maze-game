...
function graph.create(V, directed)
	...
	g.vertexList = require('assets.Modules.List').create()
	g.adjList = {}
	for v = 0,V-1 do
		g.vertexList:add(v)
		g.adjList[v] = require('assets.Modules.List').create()
	end
	g.directed = directed
...
function graph:addVertexIfNotExists(v)
	if self.vertexList:contains(v) then
		...
		self.adjList[v] = require('assets.Modules.List').create()
		return true
	end
end
...
function graph:edges()
	local list = require('assets.Modules.List').create()
